...
def test_ensure_agent_home_provisions_missing_home():
    ...
    with tempfile.TemporaryDirectory(prefix="verify_home_") as d:
        ...
        with _env(VERIFY_AGENT_HOME=str(home)):
            ...
            # they point at the repo's canonical sources (on Windows the
            # symlink fallback is a content copy, so compare contents)
            if os.name == "nt":
                assert agents_md.read_text(encoding="utf-8") == (launcher._REPO_ROOT / "agents" / "contracts" / "verifier.md").read_text(encoding="utf-8")
