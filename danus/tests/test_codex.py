...
def test_resolve_bin_falls_back_to_wrapper_then_which_then_bare():
    ...
    with env(**_ALL):
        ...
        if wrapper.exists():
            ...
            npm_js = codex._npm_codex_js()
            assert got == (str(npm_js) if npm_js else (shutil.which("codex") or "codex"))
        else:
...
def test_resolve_bin_bare_when_nothing_available(monkeypatch=None):
    ...
    real_npm = codex._npm_codex_js
    shutil.which = lambda *a, **k: None  # type: ignore[assignment]
    codex._npm_codex_js = lambda: None  # Windows: no npm-installed codex.js either
    try:
        with env(**_ALL):
...
def test_subprocess_env_prepends_dir_for_concrete_path():
    with env(**{**_ALL, "PATH": os.pathsep.join(["/usr/bin", "/bin"])}):
        out = codex.subprocess_env("/opt/codex/bin/codex")
        assert out["PATH"].split(os.pathsep)[0] == os.path.dirname(os.path.abspath("/opt/codex/bin/codex"))
...
def test_subprocess_env_never_injects_cwd_for_bare_codex():
    with env(**{**_ALL, "PATH": os.pathsep.join(["/usr/bin", "/bin"])}):
        ...
        # CWD ("" / ".") is NOT injected.
        assert out["PATH"] == os.pathsep.join(["/usr/bin", "/bin"])
        assert "" not in out["PATH"].split(os.pathsep)
        assert "." not in out["PATH"].split(os.pathsep)


def test_subprocess_env_idempotent_when_dir_already_on_path():
    codex_dir = os.path.dirname(os.path.abspath("/opt/codex/bin/codex"))
    with env(**{**_ALL, "PATH": os.pathsep.join([codex_dir, "/usr/bin"])}):
        out = codex.subprocess_env("/opt/codex/bin/codex")
        # already present → not duplicated
        expected = os.pathsep.join([os.path.dirname(os.path.abspath("/opt/codex/bin/codex")), "/usr/bin"])
...
def test_exec_cmd_empty_tail():
    ...
    tail = ["exec", "--model", "m", "--config", 'model_reasoning_effort="e"']
    if os.name == "nt":
        npm_js = codex._npm_codex_js()
        if npm_js:
            assert cmd == [codex._node_bin(), str(npm_js)] + tail
        else:
            assert cmd == ["codex"] + tail
    else:
        assert cmd == ["codex"] + tail
