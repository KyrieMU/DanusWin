...
def test_subprocess_env_prepends_dir_for_concrete_path():
    ...
    assert env["PATH"].split(os.pathsep)[0] == os.path.dirname(os.path.abspath("/opt/tools/codex"))
...
def _no_override_no_wrapper(which_result):
    ...
    orig_root, orig_which, orig_npm = codex._REPO_ROOT, shutil.which, codex._npm_codex_js
    try:
        with tempfile.TemporaryDirectory() as d:
            codex._REPO_ROOT = Path(d)  # a dir with no bin/codex wrapper
            shutil.which = lambda name: which_result
            if os.name == "nt":
                codex._npm_codex_js = lambda: None  # keep the fallback chain pure
            return codex.resolve_bin()
    finally:
        codex._REPO_ROOT, shutil.which, codex._npm_codex_js = orig_root, orig_which, orig_npm
