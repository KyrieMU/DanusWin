...
def _write_fake_codex(tmp: Path, body: str) -> Path:
    ...
    p = tmp / ("fake_codex.py" if os.name == "nt" else "fake_codex")
    p.write_text(("" if os.name == "nt" else "#!/usr/bin/env python3\n") + body,
                 encoding="utf-8")
    if os.name != "nt":
        p.chmod(0o755)
    return p
...
def test_main_sigterm_handler(tmp: Path):
    if os.name == "nt":
        return  # os.kill(SIGTERM) kills outright on Windows; handler delivery is POSIX-only
    wl = _mk_worker(tmp)
