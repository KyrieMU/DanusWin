...
try:
    import fcntl  # POSIX single-instance file lock (CLI start guard)
except ImportError:  # Windows: the desktop app supervisor has its own pid/cmdline guard
    fcntl = None
...
# Windows has no signal.SIGKILL (only SIGTERM/CTRL_*); psutil's kill() is
# the portable hard-stop for the force path.
...
def _alive(pid: Optional[int]) -> bool:
    if not pid:
        return False
    if os.name == "nt":
        # Windows has no signal-0 probe and no /proc; psutil is the portable
        # liveness check (the desktop app supervisor uses the same helper).
        try:
            import psutil
...
def _signal_tree(pid: int, sig: int) -> None:
    """Signal a worker loop and its codex child.

    POSIX: signal the loop's process group. Windows has no process
    groups, so terminate the whole process tree via psutil (the same
    helper the desktop app supervisor uses); otherwise an in-flight
    codex child would survive and keep log files locked."""
