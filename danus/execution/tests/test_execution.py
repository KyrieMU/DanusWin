...
def test_do_new_scaffolds_project(tmp: Path):
    with _project_env(tmp):
        ...
        for w, eff in [("high", "high"), ("high2", "high"), ("xhigh", "xhigh")]:
            ...
            # symlinks resolve to the (stub) contract + skills on POSIX; on
            # Windows the scaffold falls back to a copy (same content)
            if os.name == "nt":
                assert (wl.dir / "AGENTS.md").read_text(encoding="utf-8") == L.worker_md().read_text(encoding="utf-8")
                assert (wl.dir / ".agents" / "skills").is_dir()
            else:
                assert (wl.dir / "AGENTS.md").resolve() == L.worker_md().resolve()
                assert (wl.dir / ".agents" / "skills").resolve() == L.worker_skills_dir().resolve()
            cfg = wl.codex_config.read_text(encoding="utf-8")
            ...
            # the project dir is TOML-escaped (backslashes doubled on Windows)
            assert str(pdir).replace("\\", "\\\\") in cfg
            role = wl.role.read_text(encoding="utf-8")
            assert f"REASONING_EFFORT={eff}" in role and "MODEL=gpt-5.5" in role
            assert "(unassigned" in wl.task.read_text(encoding="utf-8")
            assert json.loads(wl.status.read_text(encoding="utf-8"))["state"] == "created"
...
def test_do_new_verify_url_from_env(tmp: Path):
    with _project_env(tmp):
        ...
        cfg = L.WorkerLayout(L.worker_dir("Q", "high")).codex_config.read_text(encoding="utf-8")
