...
def test_stop_one_force_sigkill_fallback(tmp: Path):
    ...
    with _project_env(tmp):
        ...
        try:
            ...
            assert res == "killed"
            if os.name != "nt":
                # Windows TerminateProcess cannot be ignored, so there is no grace window
...
def test_stop_one_force_sigkill_killpg_raises(tmp: Path):
    if os.name == "nt":
        return  # process groups (killpg) are POSIX-only
...
def test_alive_proc_read_failure_defaults_alive(tmp: Path):
    if os.name == "nt":
        return  # /proc zombie semantics are POSIX-only
...
def test_stop_one_force_getpgid_raises(tmp: Path):
    if os.name == "nt":
        return  # os.getpgid is POSIX-only
...
def test_alive_permission_error_means_alive():
    if os.name == "nt":
        return  # signal-0 ownership semantics are POSIX-only
    ...
    if not hasattr(os, "geteuid") or os.geteuid() == 0:
        return  # POSIX-only semantics
    assert cli._alive(1) is True


def test_alive_zombie_is_dead():
    if os.name == "nt":
        return  # fork()/zombie reaping is POSIX-only
...
def test_do_start_locked_returns_locked(tmp: Path):
    if os.name == "nt":
        return  # flock is POSIX-only; the Windows app supervisor has its own guard
    import fcntl
